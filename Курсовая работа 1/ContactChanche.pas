...
uses AddAlarmClock, MainMenu, WorkingWithContacts;
...
begin
  ...
  for i := 0 to length(Number_Phone.Text)-1 do
  begin
    if (ord(Number_Phone.Text[i+1]) in [48..58]) or  (Number_Phone.Text[i+1] = '+') then
     begin
       continue;
     end
     else
     begin
        Showmessage('Номер не может включать символы, отличные от цифр и символа +');
        flag := false;
        Setlength(MyContacts, length(MyContacts)-1);
        break;
     end;
  ...
  if Flag = true then
   begin
     ...
     Showmessage('Новый контакт успешно добавлен!');
   end;
end;
...
begin
  ...
  begin
    if (MyContacts[i].Contact_Name = Name_Contact.Text) and (MyContacts[i].Contact_Number = Number_Phone.Text) and flag = false then
    begin
      flag := true;
    end;
  end;
