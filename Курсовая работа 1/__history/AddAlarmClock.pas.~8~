  ...
  begin
    ...
    begin
      try
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Showmessage('Будильник успешно добавлен!');
            end
            else
            begin
        ...
        Showmessage('Время должно быть числом!');
      end;
    end;
  end
  else
  begin
    Showmessage('Имя слишком длинное!');
  end;
end;
...
begin
  ...
  Showmessage('Будильников нет, попробуйте добавить новый!')
  else
  for i := 0 to length(Alarm_Clock) -1 do
    begin
      ...
      begin
        ...
        if (Day_Of_Week.text = 'Воскресенье') xor (Day_Of_Week.text = 'Понедельник') xor
        (Day_Of_Week.text = 'Вторник') xor (Day_Of_Week.text = 'Среда') xor
        (Day_Of_Week.text = 'Четверг') xor (Day_Of_Week.text = 'Пятница') xor (Day_Of_Week.text = 'Суббота') then
        begin
          if (StrToInt(Hour.Text) >= 0) and (StrToInt(Hour.Text)<24) and (StrToInt(minute.Text)>=0) and (StrToInt(minute.Text)<60) then
          begin
            if (((compareDateTime(now, EncodeDateTime(YearOf(now), MonthOf(now), DayOfTheMonth(now), StrToInt(Hour.Text), StrToInt(Minute.Text), 0, 0)) <1) and(Day_Of_Week.Text = Day[DayOfWeek(now)])) or (Day_Of_Week.Text <> Day[DayOfWeek(now)])) then
      ...
      Showmessage('Будильника, который вы хотите изменить не существует!');
      end;
    end;
end;
...
begin
  ...
  for i := 0 to length(Alarm_Clock)-2 do
   ...
   Showmessage('Такого будильника нет!');
end;
